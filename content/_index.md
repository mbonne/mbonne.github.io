...
**Automation and scripting:** shell scripting patterns that hold up over time, approaches to keeping infrastructure reproducible, and the unglamorous work of making a homelab run reliably without constant manual intervention.

**AI tooling:** running local language models with practical infrastructure behind them, integrating LLMs into real workflows, and lessons from using AI-assisted development tooling seriously rather than experimentally.

**Security and networking:** hardening public-facing services, Cloudflare configuration, reverse proxy patterns with Traefik, and authentication with Authelia. The focus is on realistic threat models for personal infrastructure exposed to the internet, not enterprise compliance theatre.
...
New articles appear when there is something worth writing about: a problem that took longer than it should have to solve, a configuration that is not well documented anywhere, or a decision that required weighing trade-offs that do not show up in quick-start guides.

No publishing schedule. No padding to hit a quota. Posts that are worth reading, when there is something worth saying.
